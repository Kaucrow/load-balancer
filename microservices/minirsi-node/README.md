...
## Resumen
- `compiler/Compiler.js`: genera stubs para servidor y cliente a partir de `compiler/proto.txt`.
...
## Estructura
- `compiler/proto.txt`: definición del servicio.
...
- Request: una línea JSON con `class`, `method`, `params`.
- Response: una línea JSON con `status` (`ok|error`) y `response` o `msg`.
